...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        ///   <param start="name"> Starting cell to cells that need to be recalculated.</param>
        ...
        private void Visit(String start, String name, ISet<String> visited, LinkedList<String> changed)
        {
            ...
            {
                // if a cell depends on itself through a CircularException
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                // mark cell as visited and recursively visit each cell
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            // add each dependent cell to changed
